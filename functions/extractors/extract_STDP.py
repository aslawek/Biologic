...
def extract_STDP(data, number_of_sequences):
    ...
    for cycle in range(int(data['read_cycle'].max() / 6)):
        ...
        if cycle < number_of_sequences: #if changed in the future, one should modify the whole function to take number_of_sequences variable from the main function
...
def extract_STDP_1xREAD(data, number_of_sequences):
    # For read_cycles
    data_STDP = pd.DataFrame({
        'cycle': pd.Series(dtype='int32'),
        'READ_before_1/mA': pd.Series(dtype='float64'),
    ...
    for cycle in range(int(data['read_cycle'].max() / 2)):
        ...
        if cycle < number_of_sequences: #if changed in the future, one should modify the whole function to take number_of_sequences variable from the main function
            data_STDP = data_STDP.append({
                'cycle': cycle+1,
                'READ_before_1/mA': data.loc[data['read_cycle'] == (cycle * 2 + 1)]['WE(1).Current (A)'].median(),
                'READ_after_1/mA': data.loc[data['read_cycle'] == (cycle * 2 + 2)]['WE(1).Current (A)'].median(),
            }, ignore_index=True)
            ...
            columns_before = [1] #columns of READ_before_1
            selected_before = data_STDP.iloc[row, columns_before]
            # print('data', data_STDP.iloc[row, columns_before])
            read_before_mean = selected_before.values.mean()
            # print('mean seleceted before',  read_before_mean)
            ...
            columns_after = [2] #columns of READ_after_1 , 2 and 3
            selected_after = data_STDP.iloc[row, columns_after]
            read_after_mean = selected_after.values.mean()
            # append to data_STDP
            data_STDP.at[row, 'READ_after_MEAN/mA'] = read_after_mean
