...
def load_from_txt(filename, bsl_corr):
    rows_to_skip = 0
    ...
    # Acutally loading of the data #[['Time(s)',  'WE(1).Current(A)',   'WE(1).Potential(V)',   'Corrected time(s)']]:
    data = pd.read_csv(f'{filename}',  skiprows=rows_to_skip, sep='\t')

    # Poor version of background correction:
